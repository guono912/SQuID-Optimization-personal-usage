#!/usr/bin/env python3
"""
MHD Gate — DESC force-balance + ballooning + effective ripple.

Runs on checkpoint wout files from SQuID optimization.
Used as outer validation layer, not inner objective.

Usage:
    python scripts/mhd_gate.py --wout path/to/wout.nc --output_dir gate_results/
    python scripts/mhd_gate.py --wout path/to/wout.nc --compare baseline.json
"""
...
def _compute_effective_ripple(eq, s_vals=None):
    ...
    return {"per_surface": results, "peak": peak, "elapsed_s": elapsed}


def gate(wout_path, output_dir="gate_results", L=6, M=6, N=6,
    ...
    mask = (s_dm >= 0.1) & (s_dm <= 0.97) & np.isfinite(dmerc)
    vmec_info["DMerc_min_gated"] = float(np.min(dmerc[mask]))
    vmec_info["DMerc_negative_count"] = int(np.sum(dmerc[mask] < 0))
    ...
    print(f"  VMEC: ier={vmec_info['ier_flag']}, β={vmec_info['betatotal']*100:.2f}%, "
          ...
          f"DMerc_min={vmec_info['DMerc_min_gated']:.3f}, "
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="MHD Gate: DESC force-balance + ballooning + ripple")
