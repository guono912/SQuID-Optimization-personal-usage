...
def _desc_ballooning(wout, rho, alpha, nturns, nzetaperturn, nzeta0, L, M, N):
    ...
    for j, a in enumerate(alpha):
        ...
        lam[:, j] = np.asarray(obj.compute(eq.params_dict), dtype=float)
    return lam
...
def main():
    ...
    rho_grid = np.sqrt(s)
    dmerc = _array(ds, "DMerc")
    dwell = _array(ds, "DWell")
    dgeod = _array(ds, "DGeod")
    ...
    alpha = np.linspace(0, np.pi, args.nalpha)
    ...
    ax[0, 0].plot(s, dmerc, marker="o", label="DMerc")
    ax[0, 0].axhline(0, color="k", lw=0.8)
    ax[0, 0].axvspan(0, 0.1, color="0.9", label="axis-near excluded")
    ax[0, 0].set_xlabel("s = normalized toroidal flux")
    ax[0, 0].set_ylabel("DMerc [dimensionless]")
    ax[0, 0].set_title("Mercier stability profile")
    ax[0, 0].legend(fontsize=8)

    ax[0, 1].plot(s, 100 * well_depth, marker="o", label="well depth")
    if dwell is not None:
        ax2 = ax[0, 1].twinx()
        ax2.plot(s, dwell, color="tab:orange", marker="s", alpha=0.75,
                 label="DWell")
        ax2.set_ylabel("DWell [dimensionless]")
    ...
    for val, label, m in _dangerous_rationals(4, 16):
    ...
    ax[1, 0].set_title("nfp=4 filtered rational placement")
    ...
    im = ax[0, 0].imshow(
        ...
        origin="lower",
        aspect="auto",
        extent=[alpha[0], alpha[-1], rho[0], rho[-1]],
    ...
    fig.colorbar(im, ax=ax[0, 0], label="positive lambda [DESC normalized units]")

    ax[0, 1].plot(rho, np.nanmax(ballooning, axis=1), marker="o")
    ax[0, 1].axhline(0, color="k", lw=0.8)
    ax[0, 1].set_xlabel("rho = sqrt(s)")
    ax[0, 1].set_ylabel("max positive lambda [DESC units]")
    ...
    ax[1, 0].set_ylabel("effective ripple epsilon_eff^(3/2) [dimensionless]")
    ...
    ballooning_by_rho = [
        {"rho": float(r), "lambda_positive_max": float(v)}
    ...
    summary = {
        ...
        "mhd": {
            "mercier_min_s_ge_0p1": float(np.nanmin(dmerc[(s >= 0.1) & (s <= 0.97)])),
            "mercier_negative_count_s_ge_0p1": int(np.sum(dmerc[(s >= 0.1) & (s <= 0.97)] < 0)),
        ...
        "desc_ballooning": {
            ...
            "rho": rho.tolist(),
            "alpha": alpha.tolist(),
            "lambda_positive": ballooning.tolist(),
            "lambda_positive_max_by_rho": ballooning_by_rho,
            "lambda_positive_max": float(np.nanmax(ballooning)),
            "positive_count": int(np.sum(ballooning > 0)),
        },
        "desc_newcomb": {
        ...
        if p.exists():
            ...
            report["physical_validation"] = {
                ...
                "desc_ballooning": {
                    ...
                    "lambda_positive_max_by_rho": ballooning_by_rho,
                    "lambda_positive_max": summary["desc_ballooning"]["lambda_positive_max"],
                    "positive_count": summary["desc_ballooning"]["positive_count"],
                },
                "desc_effective_ripple": {
                    "epsilon_eff_by_rho": ripple_by_rho,
