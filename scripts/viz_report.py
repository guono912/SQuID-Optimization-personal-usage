#!/usr/bin/env python3
"""
SQuID configuration diagnostic report.
...
def plot_mhd_stability(vmec, eq_desc, nfp, s_gate=0.05, output_path=None,
    ...
    wout = vmec.wout
    dmerc = np.array(wout.DMerc)
    s_dm = np.linspace(0, 1, len(dmerc))
    ...
    if np.isfinite(yl) and yl > 1e-6:
        ax.set_ylim(-yl, yl)
    ax.set_xlabel("s"); ax.set_ylabel(r"$D_{\rm Merc}$")
    ax.set_xlim(s_gate, 1.0)
    ax.grid(True, alpha=0.25)
...
def build_scalar_json(nc_file, vmec, eq_desc, info, mercier_facts, well_facts,
    ...
    B0_est = float(wout.phi[-1]) / (np.pi * float(wout.Aminor_p)**2)
    # Mean |B| on axis ≈ B0 (toroidal field dominates at axis)
    b_axis_mean = B0_est
    b_axis_min = B0_est * (1.0 - info["mirror_ratio"]/2)
    b_axis_max = B0_est * (1.0 + info["mirror_ratio"]/2)

    # ── B0 (ISS04-compatible: toroidal flux / cross-section area) ──
    B0_iss04 = float(wout.phi[-1]) / (np.pi * float(wout.Aminor_p)**2)
    ...
    # ── LCFS |B| ──
    from squid.evaluation.evaluate import run_boozer as _rb, reconstruct_B as _rcB
    ...
    for iota_val, m, k in _nfp_dangerous_rationals(
        nfp, float(wout.iotaf[0])-0.01, float(wout.iotaf[-1])+0.01, m_max=16):
        if float(wout.iotaf[0]) <= iota_val <= float(wout.iotaf[-1]):
    ...
    return {
        ...
        "surface_area_m2": float(wout.Aminor_p * 2*np.pi * float(wout.Rmajor_p) * 2*np.pi * 2),  # 2πR × 2πa × 2 approx
        ...
        # MHD stability
        "DMerc_min_gated": mercier_facts.get("min"),
        "DMerc_negative_count_gated": mercier_facts.get("negative_count", 0),
        "DMerc_exclude_below_s": s_gate,
...
def generate_report(nc_file, output_dir="viz", nfp=4, gate_json=None):
    ...
    # Core SQuID
    from squid.evaluation.evaluate import evaluate_squid_detailed
    ...
    dmerc = np.array(wout.DMerc)
    s_dm = np.linspace(0, 1, len(dmerc))
    mask = (s_dm >= s_gate) & (s_dm <= 0.97) & np.isfinite(dmerc)
    v_m = dmerc[mask]
    mercier_facts = {"min": float(np.min(v_m)) if len(v_m) else np.nan,
                     "negative_count": int(np.sum(v_m < 0))}
    ...
    # J contours
    from squid.evaluation.evaluate import plot_J_contours
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", default="viz")
