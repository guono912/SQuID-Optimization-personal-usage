...
from squid.diagnostics.iota_rationals import (  # noqa: E402
    _scan_iota_rationals,
)
from squid.diagnostics.mercier_normalization import (  # noqa: E402
    MERCIER_CONVENTION_VERSION,
    edge_toroidal_flux_wb,
    flux_normalize_mercier,
)
from squid.utils.jax_runtime import initialize_requested_jax_backend  # noqa: E402
...
def _evaluate_effective_ripple(vmec, s_val, nc_file_path, output_dir="."):
    ...
    try:
        ...
        from squid.utils import nc_to_neor
        import subprocess
        ...
        in_file_path, epsmn, pert_m0, pert_mph = resul
...
# Low-order rational scan moved to squid/diagnostics/iota_rationals.py
...
def _mercier_summary(mercier_data):
    ...
    neg = finite_values < 0
    raw_values = np.asarray(mercier_data.get("raw_values", values), dtype=float)[finite]
    return {
        "available": True,
        "convention": MERCIER_CONVENTION_VERSION,
        "formula": "Phi_edge^2 * VMEC.DMerc",
        "edge_toroidal_flux_wb": float(mercier_data["edge_toroidal_flux_wb"]),
        "flux_normalized_min": float(finite_values[min_idx]),
        "vmec_raw_min": float(raw_values[min_idx]),
...
def _basic_configuration_summary(
    ...
    out["R0_over_a_aspect_ratio"] = aspec
    ...
    if mercier_facts and mercier_facts.get("available"):
        out["dmerc_convention"] = mercier_facts["convention"]
        out["dmerc_edge_toroidal_flux_wb"] = float(mercier_facts["edge_toroidal_flux_wb"])
        out["dmerc_vmec_raw_min"] = float(mercier_facts["vmec_raw_min"])
        out["dmerc_flux_normalized_min"] = float(mercier_facts["flux_normalized_min"])
        out["dmerc_s_at_min"] = float(mercier_facts["s_at_min"])
        out["dmerc_negative_count"] = int(mercier_facts["negative_count"])
...
def _compact_diagnostic_report(
    ...
    report = {
        ...
        "profiles": {
            ...
            "Mercier_flux_normalized": _compact_profile(
                None if mercier_data is None else mercier_data.get("s"),
                None if mercier_data is None else mercier_data.get("values"),
                "s", "Phi_edge^2_DMerc"),
            "Mercier_VMEC_raw": _compact_profile(
                None if mercier_data is None else mercier_data.get("s"),
                None if mercier_data is None else mercier_data.get("raw_values"),
                "s", "DMerc_VMEC_raw"),
...
def main():
    ...
    parser.add_argument("--mhd_s_max", type=float, default=0.97,
                        help="Maximum s used for Mercier/MHD gate summaries")
    ...
    try:
        if hasattr(vmec.wout, "DMerc"):
            ...
            dmerc_raw = vmec.wout.Dmerc
        dmerc_raw_full = np.array(dmerc_raw[1:], dtype=float)
        dmerc_full = flux_normalize_mercier(dmerc_raw_full, vmec.wout)
        s_half_full = np.array(vmec.s_half_grid, dtype=float)
        mhd_mask = (
            (s_half_full >= args.mhd_s_min)
            & (s_half_full <= args.mhd_s_max)
        )
        ...
        mercier_data = {
            "s": s_half,
            "values": dmerc,
            "raw_values": dmerc_raw_full[mhd_mask],
            "edge_toroidal_flux_wb": edge_toroidal_flux_wb(vmec.wout),
        }
        mercier_facts = _mercier_summary(mercier_data)
        print(
            f"  Mercier Phi_edge^2*D_Merc "
            f"({args.mhd_s_min:g}<=s<={args.mhd_s_max:g}): "
            f"min={mercier_facts['flux_normalized_min']:.4e} "
            f"(raw={mercier_facts['vmec_raw_min']:.4e}) "
            f"at s={mercier_facts['s_at_min']:.2f}, "
