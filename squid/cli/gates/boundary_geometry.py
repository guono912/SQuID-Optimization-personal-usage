"""Boundary-geometry gate CLI implementation.

Computes pdrot/principal-curvature coil-hardness proxies for checkpoint wouts
and writes a summary CSV. The heavy metrics come from
squid/diagnostics/boundary_geometry.py and squid/objectives/pdrot_residual.py;
this module only parses the CLI, drives the loop, and prints the report.
"""
...
def main(argv=None) -> None:
    ...
    for wout in wouts:
        ...
        boundary_pdrot_aw = _stat(metrics, "pdrot_1_per_m", "mean_area_weighted")
        boundary_pdrot_max = _stat(metrics, "pdrot_1_per_m", "max")
        surface = SurfaceRZFourier.from_wout(
            str(wout), range="full torus", nphi=args.nphi, ntheta=args.ntheta
        )
        pdrot_stats = pdrot_area_weighted_stats(
            principal_direction_rotation_rate(surface)
        )
        pdrot_aw = float(pdrot_stats["pdrot_mean"])
        pdrot_max = float(pdrot_stats["pdrot_max"])
        pdrot_p95 = float(pdrot_stats["pdrot_p95"])
        pdrot_p99 = float(pdrot_stats["pdrot_p99"])
        pdrot_p999 = float(pdrot_stats["pdrot_p999"])
        pdrot_cvar1 = float(pdrot_stats["pdrot_cvar1"])
        ...
        rows.append(
            {
                ...
                "pdrot_p95": pdrot_p95,
                "pdrot_p99": pdrot_p99,
                "pdrot_p999": pdrot_p999,
                "pdrot_cvar1": pdrot_cvar1,
                "pdrot_max": pdrot_max,
                "boundary_geometry_pdrot_aw_legacy": boundary_pdrot_aw,
                "boundary_geometry_pdrot_max_legacy": boundary_pdrot_max,
                "k2_min": k2_min,
                "H_min": h_min,
    ...
    for row in rows:
        print(
            ...
            f"pdrot_aw={row['pdrot_aw']:.4f} pdrot_p99={row['pdrot_p99']:.2f} "
            f"pdrot_max={row['pdrot_max']:.2f} "
