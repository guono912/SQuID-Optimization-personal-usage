"""Batch MHD gate CLI implementation for SQuID checkpoint wout files.
...
The promotion gate itself lives in squid/evaluation/gates.py.

Usage:
    python scripts/gate/gate_checkpoints.py --run_dir runs/edge_bal_repair
    python scripts/gate/gate_checkpoints.py --run_dir runs/edge_bal_repair --watch
"""
...
def gate_checkpoints(run_dir, output_dir=None, watch=False, min_interval=60):
    ...
    header = ("wout,ier_flag,aspect,betatotal,iota_axis,iota_edge,"
              ...
              "dmerc_flux_normalized_min,dmerc_neg,well_depth,fb_rms_final,"
    ...
    if csv_path.exists():
        with open(csv_path) as f:
            ...
            if existing_header != header:
                raise RuntimeError(
                    f"{csv_path} uses a pre-flux-normalization schema; "
                    "archive it and start a new gate_summary.csv"
                )
            for line in f:
        ...
        for wf in wout_files:
            ...
            try:
                ...
                row = (f"{stem},{v['ier_flag']},{v['aspect']:.4f},{v['betatotal']:.6f},"
                       ...
                       f"{v['dmerc_edge_toroidal_flux_wb']:.8e},"
                       f"{v['dmerc_vmec_raw_min_gated']:.8e},"
                       f"{v['dmerc_flux_normalized_min_gated']:.8e},"
                       f"{v['dmerc_negative_count']},"
